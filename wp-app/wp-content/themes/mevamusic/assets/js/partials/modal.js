...
$('.js-service-card_btn').each(function () {
    ...
    let cardDescriptionList = cardContainer.find('.js-more-modal__list');
    if (cardDescriptionList.length) {
        let outerHtml = cardDescriptionList.prop('outerHTML');
        if (outerHtml) {
            cardDescriptionList = outerHtml.replace(/visually-hidden/g, '')
        }
    }
